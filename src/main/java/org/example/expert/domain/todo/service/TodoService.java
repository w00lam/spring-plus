...
import org.example.expert.domain.todo.dto.request.TodoGetCondition;
...
public class TodoService {
	...
	public Page<TodoResponse> getTodos(TodoGetCondition condition, int page, int size) {
		validatePageRequest(page, size);
		validateModifiedAtRange(condition);

		Pageable pageable = PageRequest.of(page - 1, size);

		Page<Todo> todos = todoRepository.getTodosWithCondition(
	...
	public Page<TodoSearchResponse> searchTodos(TodoSearchCondition condition, int page, int size) {
		validatePageRequest(page, size);
		validateCreatedAtRange(condition);
	...
	private void validateCreatedAtRange(TodoSearchCondition condition) {
		if (condition.getCreatedAtFrom() != null
			&& condition.getCreatedAtTo() != null
			&& condition.getCreatedAtFrom().isAfter(condition.getCreatedAtTo())) {
			throw new InvalidRequestException("생성일 시작일은 종료일보다 이후일 수 없습니다.");
		}
	}

	private void validateModifiedAtRange(TodoGetCondition condition) {
